...
| `EVOLUTION.md` + `evolution/` | **실무형 진화**: 단순 큐 1개 → 여러 서비스로 커지는 과정을 S0~S5 실행 파일로 단계별 체험 |
| `production/` | **창업하면 실제로 쓸 표준 솔루션** + 코드 (SQS / RabbitMQ / Redis / Kafka, Spring Boot) |
...
| `tests/` | **실무자 레이어**: 그 모듈 코드의 동작을 고정하는 실행되는 테스트 스위트 (unit + 동시성 불변식 + 경계). 주석은 영어 |
| `debug/` + `DEBUG.md` | **실무자 레이어**: 심어둔 프로덕션급 버그 + 6단계 디버깅 루프(증상→결정론적 재현→국소화→근본원인→수정→재발방지) |
| `CHANGE.md` | **실무자 레이어**: 낯선 코드에 최소 안전 변경으로 기능 추가 + 증명 테스트 + 영어 PR 설명 |

> **2일 리듬(Day A / Day B):** 한 모듈 = 약 이틀.
> **Day A** = 이론·진화·레퍼런스·백지·정답(공부한 사람).
> **Day B** = `tests/`·`debug/`·`CHANGE.md`(실무자). 같은 코드 위에서
> 테스트·디버깅·안전변경·영어 글쓰기를 *그 주제로* 익힌다. 별도 트랙이
> 아니라 모듈 안에 융합 — 토이가 아닌 진짜 코드 위에서만 늘기 때문.

> 코드 규약: 학습자가 미국에서 일하므로 **모든 코드 주석은 단순하고 명확한 영어**로 작성한다.
> 실행 가능한 코드(`reference/`,`evolution/`,`blank/`,`solution/`,`tests/`,`debug/`)는 작성 후 반드시 컴파일·실행·검증하고,
> 동시성 코드는 여러 번 반복 실행해 안정성을 확인한다. `tests/`는 `java <file>`로 돌려 초록/0-exit, `debug/`는 버그가 재현됨을 확인.
>
> 문서 규약: `THEORY.md`·`QUIZ.md`는 **한글↔영어 병기**(개념은 모국어로 잡고 바로 영어로 다시 보기).
> `production/`은 실무 표준 제품 사용 코드(Maven/서버 필요 → 레포 내 실행 X, 참조용).
